...
fn run_coordination_strategy(repetitive: bool) -> serde_json::Value {
    ...
    with_creation_daemon_seed_backend(
        |_, _| (),
        |backend| backend.fixture_with_unsteerable_parent(),
        |client, _, root, repository, address, ()| {
            ...
            .unwrap();
            std::fs::write(
                root.join("claude-child-fixture"),
                include_str!("../tests/fixtures/steward_coordination.py"),
            )
            .unwrap();
            ...
            let before_discussion = settled(&client, parent.id, |_| true);
            let discussion = client.request(Uuid::nil(), Uuid::nil(), Command::Consult {source_session_id:parent.id, question:"Can we change the dependent output while preserving the current feature work?".into()}).unwrap();
            let discussion = serde_json::to_value(discussion).unwrap();
            assert_eq!(
                discussion["consultation"]["exchanges"]
                    .as_array()
                    .unwrap()
                    .len(),
                1
            ...
            assert_eq!(
                discussion["consultation"]["exchanges"][0]["answer"],
                "Discussed only; no task direction has been changed."
            );
            let after_discussion = settled(&client, parent.id, |_| true);
            assert_eq!(
                serde_json::to_value(&before_discussion.messages).unwrap(),
                serde_json::to_value(&after_discussion.messages).unwrap()
            );
            assert_eq!(
                serde_json::to_value(&before_discussion.turns).unwrap(),
                serde_json::to_value(&after_discussion.turns).unwrap()
            );
            assert_eq!(
                before_discussion.steward_wait,
                after_discussion.steward_wait
            );
            assert_eq!(
                before_discussion.input_deliveries,
                after_discussion.input_deliveries
            );
            let direction_id = Uuid::new_v4();
            let direction = "Change dependent.txt acceptance to direction-approved result; preserve feature.txt work.";
            let queued = client
                .request(
                    Uuid::nil(),
                    Uuid::nil(),
                    Command::ExecuteConsultation {
                        ...
                        delivery_id: direction_id,
                        instruction: direction.into(),
                    },
                )
                .unwrap();
            let queued = serde_json::to_value(queued).unwrap();
            assert_eq!(
                queued["consultation"]["instructions"][0]["delivery"]["state"],
                "queued"
            );
            assert_eq!(
                queued["consultation"]["instructions"][0]["pending_targets"][0]["session_id"],
                implementer.to_string()
            );
            assert!(
                !root.join("direction-prompts.jsonl").exists(),
                "queued input must not cross the provider boundary yet"
            );
            std::fs::write(root.join("finish-parent"), "").unwrap();
            let directed = settled(&client, parent.id, |s| {
                s.turns.len() == 2
                    && root.join("direction-prompts.jsonl").exists()
                    && s.input_deliveries.iter().any(|d| {
                        d.id == direction_id
                            && d.state == crate::model::InputDeliveryState::Received
                    })
            });
            assert!(directed.active_turn_id().is_some());
            assert!(directed.steward_wait.is_none());
            ...
            .unwrap();
            assert_eq!(
                received["consultation"]["instructions"][0]["delivery"]["state"],
                "received"
            );
            let direction_text =
                std::fs::read_to_string(root.join("direction-prompts.jsonl")).unwrap();
            assert_eq!(direction_text.lines().count(), 1);
            assert!(direction_text.contains(direction));
            assert!(direction_text.contains(&implementer.to_string()));
            assert!(
                !root.join("callback-prompts.jsonl").exists(),
                "old wait must not race the explicit instruction"
            );
            assert_eq!(
                tools.call("waku_wait", json!({"session_ids":[implementer]}))["waiting"],
                true
            );
            std::fs::write(root.join("finish-direction"), "").unwrap();
            ...
            settled(&client, parent.id, |s| s.turns.len() == 3 && completed(s));
            let callback_text =
                std::fs::read_to_string(root.join("callback-prompts.jsonl")).unwrap();
            ...
            let baseline =
                accepted["session"]["managed_workspace"]["results"][0]["integration_commit"]
                    .clone();
            let (dependent, dependent_path) = tools.spawn(json!({"owner":"dependent","scope":"dependent.txt","baseline":baseline,"expected":"direction-approved result","acceptance":"dependent.txt contains direction-approved result; reuse accepted feature evidence only at its recorded commit"}), json!([{"session_id":implementer,"commit":fixed["commit"]}]));
            ...
            let accepted = tools.call("waku_workspace", json!({"operation":{"type":"integrate","sessionId":dependent,"commit":dependent_result["commit"],"expectedIntegrationCommit":baseline,"evidence":[{"commit":dependent_result["commit"],"checks":"dependent.txt equals direction-approved result","environment":"controlled-git-v1","reviewer":"controlled dependency check"}]}}));
            assert_eq!(
            ...
            assert_eq!(
                std::fs::read_to_string(repository.join("dependent.txt")).unwrap(),
                "direction-approved result\n"
            );
            ...
            for id in [implementer, dependent] {
                let removed = settled(&client, id, |session| {
                    session.managed_workspace.as_ref().is_some_and(|workspace| {
                        workspace.cleanup.iter().any(|item| {
            ...
            let consultation_snapshot = serde_json::to_value(
                client
                    .request(
                        Uuid::nil(),
                        Uuid::nil(),
