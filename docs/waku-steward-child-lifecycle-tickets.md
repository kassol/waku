...
首批可执行 #24、#25；其余按阻塞边完成后的可执行集合推进。不存在必须预先进行的独立重构，局部调整随所属行为票完成。实现 subagents 使用 Astra，共享源码修改按文件归属或隔离工作树协调。
...
父规格的正文、标签和开放状态保持不变；子票均标记 ready-for-agent，并通过原生子 Issue 与 blocking 关系关联。此次发布不包含实现、安装、自动整理历史子会话或删除数据。
