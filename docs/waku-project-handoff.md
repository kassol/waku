...
日期：2026-09-06。当前项目已接续交接，处于期 1 规格与任务拆分准备阶段。本文为当前入口；原始交接与设计稿中的历史状态以本文为准。
...
用户询问过 ask-matt 的路线建议。已建议 `to-spec` → `to-tickets` → 逐票 `implement`，尚未执行或产出 spec / tickets。后续先检查前置配置。

1. 检查 `setup-matt-pocock-skills` 是否已配置任务跟踪方式与文档目录，避免重复初始化。
2. 读取当前源码，核对期 1 设计中的关键事实，再将已定稿设计收敛为可验收规格。沿用原有决策；影响范围或对外行为的新决策需用户拍板。
3. 规格确认后拆分带阻塞关系的纵向任务。建议首个行为闭环：Claude 管家派生 Codex 子会话 → 查询完成状态 → 读取结果；构建基线是实现验收的前置条件。
4. 后续实现逐票完成必要回归检查与 Standards / Spec 两轴评审。

本轮阅读设计稿发现以下待复核项，均为文档层面的疑点，**尚无当前源码验证**：

- 事件队列满时丢弃的策略，与转录不丢的目标存在冲突；规格需明确持久化失败与恢复保证。
- 观测层分期、期 1 MCP 工具数量的表述前后不一致，收敛规格时统一。
- 旧客户端快照是否保留父子关系，需检查完整合并与写入路径；不能仅凭 serde 属性或陈旧快照分支推出兼容性。
